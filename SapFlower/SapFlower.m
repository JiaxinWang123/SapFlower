    ...
    methods (Access = private)
        ...
        function DriftDampingMenuSelected(app, event)
            ...
            try
                ...
                app.appendToPreprocessingLog({ ...
                    sprintf('[Drift/Damping] Detection: %s', diag.message), ...
                    sprintf('[Drift/Damping] Recommendation: %s', diag.recommendation)});
            catch ME
                app.appendToPreprocessingLog({ ...
                    sprintf('[Drift/Damping] Detection failed: %s', ME.message), ...
                    '[Drift/Damping] Falling back to manual mode selection.'});
                diag.recommendation = 'both';
                diag.message = '(detection unavailable)';
            end

            % Map recommendation to the dialog's default-button index.
            switch lower(diag.recommendation)
                case 'detrend', recIdx = 1;
                case 'damping', recIdx = 2;
                case 'both',    recIdx = 3;
                otherwise,      recIdx = 4;   % nothing detected -> default to Cancel
            end

            promptMsg = sprintf([...
                'Auto-detection result:\n   %s\n   Recommended mode: %s\n\n' ...
                'Apply Z-score correction? (window = %d d, ref = %d d)\n\n' ...
                'The recommended mode is preselected — press Enter to accept, ' ...
                'or pick a different option to override.'], ...
                diag.message, diag.recommendation, win, ref);
        ...
        function applyDriftDampingWithMode(app, chosenMode)
            ...
            try
                ...
                if ~isempty(info) && isstruct(info)
                    logLines{end+1} = sprintf( ...
                        '[Drift/Damping]   data: %.1f%% non-NaN; Z usable: %.1f%%', ...
                        100*info.pctValid, 100*info.pctZValid);
                    for k = 1:numel(info.segments)
                        s = info.segments(k);
                        if isnan(s.mu_ref)
                            logLines{end+1} = sprintf( ...
                                '[Drift/Damping]   seg %d [%d:%d]: status=%s (nValid=%d)', ...
                                k, s.a, s.b, s.status, s.nValid); %#ok<AGROW>
                        else
                            logLines{end+1} = sprintf( ...
                                ['[Drift/Damping]   seg %d [%d:%d]: status=%s, ' ...
                                 'refUsed=%d, μ_ref=%.3f, σ_ref=%.3f, nCorrected=%d'], ...
                                k, s.a, s.b, s.status, s.refUsed, ...
                                s.mu_ref, s.sd_ref, s.nCorrected); %#ok<AGROW>
                        end
                    end
                end

                if nC == 0
                    logLines{end+1} = ['[Drift/Damping] No samples were ' ...
                        'corrected. Most common cause: too few valid samples ' ...
                        'in the reference window after an install. Try ' ...
